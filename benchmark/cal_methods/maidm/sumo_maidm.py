...
from run_maidm.sampling_utils import load_trace, update_routes, update_flows, update_vtype_colors
...
maidm_support_files_dir_name = 'cal_methods/maidm/support_files'
network_file = 'sumo_test.net.xml'
network_file_path = os.path.join(shared_sim_dir, network_file)
detector_file_path = os.path.join(maidm_support_files_dir_name, 'sumo_test_detectors.xml')
# Output files for flowrouter
output_routes_path = os.path.join(maidm_support_files_dir_name, 'maidm.rou.xml')
output_flows_path = os.path.join(maidm_support_files_dir_name, 'maidm.flows.xml')
# Output files from simulation
outputs_dir = 'cal_methods/maidm/outputs'
...
# save_file_name = f"bayes_full-results_generated-{current_date}.csv"
save_file_name = f"maidm_full-results_generated-{current_date}.csv"
...
for i, f_name in enumerate(sorted(os.listdir(detector_measurements_dir))):
    # Skip non-CSV files
    if not f_name.endswith('.csv'):
        continue

    # Note: .sumocfg file will automatically set the sim's begin and end times based on the CSV file
    # inception_measurements_csv = 'detector_measurements/1121/detections_0420-0540.csv' # Example path to data on which to evaluate
    inception_measurements_csv = os.path.join(detector_measurements_dir, f_name)

    ### Modify if want to use an abridged simulation
    ...
    os.system(f"python3 {os.environ['SUMO_HOME']}/tools/detector/flowrouter.py -n {network_file_path} -d {detector_file_path} -f {inception_measurements_csv} -o {output_routes_path} -e {output_flows_path} -i {aggregation_interval} --respect-zero --revalidate-detectors")

    ### Sample the MA-IDM parameters and update the routes and flows XML files with them
    cache = "cal_methods/maidm/run_maidm/MA_IDM_pooled_car_I24-motion.pkl"    # Path to the cache file containing the MA-IDM MCMC samples
    # cache = "cal_methods/maidm/run_maidm/Bayesian_IDM_pooled_I24-Motion.pkl"    # Path to the cache file containing the Bayesian MCMC samples
    rou_xml_file_path = 'cal_methods/maidm/support_files/maidm.rou.xml'
    flows_xml_file_path = 'cal_methods/maidm/support_files/maidm.flows.xml' 
    num_vtypes_to_insert = 20                                # Number of <vType> elements that will comprise the vehicle distribution
    vtype_id_prefix = "maidm" 
    vtype_dist_id = "maidm_vtypes_distribution"
    tr = load_trace(cache)
    update_routes(rou_xml_file_path, tr, num_vtypes_to_insert, vtype_id_prefix, vtype_dist_id, car_follow_model="IDM", rand_seed=i)
    update_flows(flows_xml_file_path, vtype_dist_id)

    ### Prep simulation config file
    sumo_cmd = 'sumo'   # 'sumo' for regular running; 'sumo-gui' to see GUI 
    sumocfg_file = 'maidm.sumocfg'
    sumocfg_file_path = os.path.join(maidm_support_files_dir_name, sumocfg_file)
    ...
    if sumo_cmd == 'sumo-gui':
        ...
        update_vtype_colors(rou_xml_file_path)
    ...
    year_str = '2022'
    interval_len_in_min = 15 # Frequency of timestamps for microscopic objective function (inclusive)
    date_str = os.path.basename(inception_measurements_csv).split('_')[2]
    micro_obj_fn_timestamps = generate_time_intervals(year_str, date_str, begin_time_in_min, end_time_in_min, interval_len_in_min)

    # Calculate microscopic error
    # TODO: Loaded cached data for INCEPTION from sumo baseline, rather than reloading it here (to save time)
    MicroEC = MicroscopicErrorCalculator(micro_obj_fn_timestamps=micro_obj_fn_timestamps,
                    ...
                    cached_traj_dir="./cal_methods/maidm/cached_trajectories/",
                    cache_traj_verbosity=True,
                    cache_traj_disable_tqdm=False
                    )

    avg_wass_dist = MicroEC.compute_headway_wasserstein_metric(plot_distributions=False)
